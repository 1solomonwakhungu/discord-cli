...
5. **Version sync**: Every file holding a version string is rewritten automatically —
   `src/discord_cli/__init__.py`, `README.md`, `commands.json`,
   `packaging/npm/package.json`, and `CITATION.cff` (see `version_variables` in
   `pyproject.toml`). The man page is regenerated by `build_command` and staged
   via `assets`, so the release commit and its tag are internally consistent.
   Do not bump these files by hand.
6. **Git tag**: A tag `v{version}` is created and pushed.
